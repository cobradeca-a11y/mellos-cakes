  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      <div className="card p-5 space-y-3">
        {supplier.contact_name && <p className="text-sm text-[var(--text-3)]">Contato: <strong>{supplier.contact_name}</strong></p>}
        <div className="flex flex-wrap gap-4">
          {supplier.phone && (
            <a href={`tel:${supplier.phone}`} className="flex items-center gap-2 text-sm text-[var(--text-3)] hover:text-brand-500">
              <Phone className="w-4 h-4 text-[var(--muted)]" /> {supplier.phone}
            </a>
          )}
          {supplier.email && (
            <a href={`mailto:${supplier.email}`} className="flex items-center gap-2 text-sm text-[var(--text-3)] hover:text-brand-500">
              <Mail className="w-4 h-4 text-[var(--muted)]" /> {supplier.email}
            </a>
          )}
        </div>
        {supplier.notes && <p className="text-sm text-[var(--text-3)] italic">{supplier.notes}</p>}
      </div>

      <div className="table-container">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border-light)]">
          <h3 className="font-semibold text-[var(--text-1)] flex items-center gap-2">
            <Package className="w-4 h-4 text-[var(--muted)]" /> Ingredientes fornecidos
          </h3>
        ...
        <table className="table">
          ...
          <tbody>
            {(ingredients ?? []).length === 0 ? (
              <tr><td colSpan={4} className="text-center py-8 text-[var(--muted)]">Nenhum ingrediente vinculado</td></tr>
            ) : (
              (ingredients ?? []).map(i => (
                <tr key={i.id}>
