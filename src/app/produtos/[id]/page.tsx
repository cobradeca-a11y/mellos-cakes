    ...
    <div className="max-w-2xl space-y-5">
      ...
      {product.description && (
        <div className="card p-5">
          <h3 className="font-semibold text-[var(--text-1)] mb-2">Descrição</h3>
          <p className="text-sm text-[var(--text-2)]">{product.description}</p>
        </div>
      )}
      ...
      {totalCost > 0 && (
        <div className="card p-5 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            <div className="bg-[var(--hover)] rounded-xl p-3">
              <p className="text-lg font-bold text-[var(--text-1)]">{formatCurrency(totalCost)}</p>
              <p className="text-xs text-[var(--text-3)]">Custo Total</p>
            </div>
            <div className="bg-[var(--hover)] rounded-xl p-3">
              <p className="text-lg font-bold text-[var(--text-1)]">{formatCurrency(product.base_price - totalCost)}</p>
              <p className="text-xs text-[var(--text-3)]">Lucro Bruto</p>
            </div>
      ...
      {(product.recipe_compositions ?? []).length > 0 && (
        <div className="table-container">
          <div className="px-5 py-4 border-b border-[var(--border-light)]">
            <h3 className="font-semibold text-[var(--text-1)]">Ficha Técnica</h3>
          </div>
