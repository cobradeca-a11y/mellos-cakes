...
import {
  ...
  Megaphone, Settings, FileText, Building2, Package, X, Menu, Flame
} from 'lucide-react'
...
const navGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/gastos',      icon: Flame,        label: 'Gastos Operacionais' },
    ],
  },
  {
    label: 'Compras',
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active ? 'bg-brand-500 text-white shadow-sm' : 'hover:bg-[var(--hover)]'
                )}
...
function SidebarContent({ onClose }: { onClose?: () => void }) {
  // Preserva a posição do scroll ao navegar
  ...
  // Restaura posição após mudança de rota
  useEffect(() => {
    if (navRef.current) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="font-display font-semibold text-base"
              style={{ color: 'var(--text-primary)' }}>Mellos</span>
            <span className="font-display font-semibold text-base text-brand-500">Cakes</span>
            <p className="text-[10px] uppercase tracking-wider"
              style={{ color: 'var(--text-muted)' }}>Gestão</p>
          </div>
        </div>
        {onClose && (
          <button onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-[var(--hover)] md:hidden">
            <X className="w-5 h-5" style={{ color: 'var(--text-muted)' }} />
          </button>
        )}
      </div>

      {/* Nav — scroll preservado via ref */}
      <nav ref={navRef} className="flex-1 overflow-y-auto py-4 px-3">
        <NavItems onClose={onClose} />
      </nav>

      {/* Footer */}
      <div className="px-3 py-4 border-t shrink-0" style={{ borderColor: 'var(--border)' }}>
        <div className="flex items-center gap-2 px-3 py-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium truncate"
              style={{ color: 'var(--text-primary)' }}>MellosCakes</p>
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Mobile drawer */}
      <aside
        className="md:hidden fixed inset-y-0 left-0 z-50 flex flex-col w-72 transition-transform duration-300"
        style={{
